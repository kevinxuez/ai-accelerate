"""Extract DOCX paragraphs without normalizing or reproducing evidence text.
...
from .contracts import ParagraphRecord, RunRecord
...
def _visible_runs(paragraph: ET.Element) -> list[ET.Element]:
    ...
    deleted = {
        id(node)
        for deletion in paragraph.findall(f".//{TAG('del')}")
        for node in deletion.iter(TAG("r"))
    }
...
def paragraph_records(path: str | Path) -> list[ParagraphRecord]:
    ...
    for index, paragraph in enumerate(root.iter(TAG("p"))):
        ...
        for run in raw_runs:
            ...
            highlight_node = (
                properties.find(TAG("highlight")) if properties is not None else None
            )
            highlight = (
        ...
        for run in runs:
            if run.highlight:
                highlights[run.highlight] = highlights.get(run.highlight, 0) + len(
                    run.text
                )
        output.append(
            ParagraphRecord(
